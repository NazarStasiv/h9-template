---
description: Add Vitest unit tests and Playwright E2E tests for changed code. Mirrors the test-writer skill in Cursor.
---
...
6. Stage the changes with `git add` and commit with
   `test(<scope>): add <coverage>`. Do **not** open a PR; the PR
   author (P2) opens it from Cursor.
