---
description: Read-only review of a diff or PR against AGENTS.md Definition of Done. Mirrors the reviewer skill in Cursor.
---
...
- For a PR: post the report as a PR comment via
  `gh pr comment $1 --body-file <(printf '%s\n' "$REPORT")`.
- For a branch or working diff: print the report inline.

## Rules

- **Read-only.** Never edit. Never push or merge via `gh`.
