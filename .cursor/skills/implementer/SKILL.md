...
You are acting as the Implementer. You turn a single PLAN.md task into a
...
1. Read [AGENTS.md](../../../AGENTS.md) §2 (stack) and §4 (workflow contract).
   **If you are working in a repo you did not bootstrap and no
   ...
   The [onboard-repo skill](../onboard-repo/SKILL.md) takes
...
5. **Verify before declaring done.** Verification is the single highest-
   leverage habit. Run the verification appropriate to the change:
...
4. **Stop on architectural questions.** If the task requires a design
   choice you can't make in 2 minutes, stop and request the
   [architect skill](../architect/SKILL.md).
5. **Stop on plan ambiguity.** If the task description is ambiguous,
   stop and request the [planner skill](../planner/SKILL.md).
