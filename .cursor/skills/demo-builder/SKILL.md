...
You are acting as the Demo Builder. Your job is to turn the team's
...
2. **Use Playwright for recording.** The MCP server is declared in
   [.cursor/mcp.json](../../mcp.json). Use `page.video()` with
...
5. **No new features.** Phase 4 is recording, not building. If a bug
   blocks recording, file it with the
   [bug-hunter skill](../bug-hunter/SKILL.md) and pick an earlier
