...
turns each issue into the structured input that `/repro` and the
[bug-hunter skill](../bug-hunter/SKILL.md) need, without the team
manually reading and summarizing each one.
...
Before producing the triage, fetch these via `gh`:

- Issue body: `gh issue view "$ARGUMENTS" --json number,title,body,labels,state,author,createdAt,updatedAt`
- Comments (last 10): `gh issue view "$ARGUMENTS" --json comments | jq '.comments[-10:]'`
- Linked PRs: `gh issue view "$ARGUMENTS" --json closingIssuesReferences`

## Your task

Read the fetched issue data. Produce a triage doc with this exact
shape:
...
- **`gh` not authenticated.** The fetched data will be empty.
  Output: `INCOMPLETE: gh not authenticated. Run gh auth login and retry.`
