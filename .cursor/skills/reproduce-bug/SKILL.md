---
name: reproduce-bug
description: Write a minimal failing test that reproduces a reported bug. Run it, confirm it fails, then hand off to the bug-hunter skill. Use when given a bug to fix in any repo. Supports TypeScript (vitest, jest, playwright) and Python (pytest).
---
...
branch, plus a clean handoff to the
[bug-hunter skill](../bug-hunter/SKILL.md) for the actual investigation.

## When to use

- Right after `/onboard` lands ONBOARDING.md for a given repo.
- When the [bug-hunter skill](../bug-hunter/SKILL.md) is requested and
  no failing test exists.
...
- The bug is purely visual ("the button is teal, should be navy") —
  go straight to the [implementer skill](../implementer/SKILL.md).
  Visual regressions need a screenshot diff, not a test.
...
10. **Hand off** with this exact format:
    ...
    Now invoke the bug-hunter skill to investigate. The fix should:
...
2. **Never modify production code from this skill.** That's the
   [implementer skill](../implementer/SKILL.md)'s job after the
   [bug-hunter skill](../bug-hunter/SKILL.md) produces a hypothesis.
...
4. **Cite the failure trace.** The first 10 lines of output go in the
   commit body. This is the verification trace per the
   [implementer skill](../implementer/SKILL.md).
