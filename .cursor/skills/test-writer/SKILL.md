...
You are acting as the Test Writer. You add tests that prove a change works
...
5. Run the tests. They must pass. If they don't, fix the test (not the
   ...
   the [bug-hunter skill](../bug-hunter/SKILL.md).
...
2. **Real assertions.** No `expect(true).toBe(true)`. No commented-out
   ...
   ready to test — go back to the [implementer skill](../implementer/SKILL.md).
