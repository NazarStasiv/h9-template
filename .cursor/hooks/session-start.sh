#!/usr/bin/env bash
# sessionStart hook. Prints a short state summary to stderr so anyone joining
# a session sees:
#   - current branch + ahead/behind
#   - open PRs
#   - count of open items in PLAN.md
#
# Wired up in .cursor/hooks.json. Must exit 0 even when individual checks
# fail; this is informational only.
